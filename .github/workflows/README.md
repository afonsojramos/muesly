...
- Versioned updater payloads and stable installer aliases are published to the public `muesly-downloads` R2 bucket at `downloads.muesly.ai`. The workflow rewrites every `latest.json` platform URL to that public origin before publishing it. `SHA256SUMS.txt` covers the public installers, and the release fails if an expected installer, updater platform, or public URL is missing.
