...
pub async fn api_generate_meeting_title<R: Runtime>(
    ...
    log_info!(
        "api_generate_meeting_title called for meeting_id: {}, model: {}",
        meeting_id,
        model
    );
    let pool = state.db_manager.pool().clone();
    SummaryService::generate_meeting_title(app, pool, meeting_id, text, model, model_name).await
}

/// Resolves a meeting's on-disk folder (where metadata.json lives), if recorded.
...
pub async fn api_get_meeting_summary_language<R: Runtime>(
    ...
    match resolve_meeting_folder(state.db_manager.pool(), &meeting_id).await? {
        Some(folder) => read_summary_language_from_metadata(&folder).map_err(|e| e.to_string()),
        None => Ok(None),
    }
}
