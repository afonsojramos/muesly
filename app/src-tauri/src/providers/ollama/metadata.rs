...
/// Process-wide Ollama model-metadata cache (5-minute TTL), shared by the
/// `ollama` command module and the `summary` service so a model's metadata is
/// fetched at most once per TTL window across the whole app.
pub static SHARED_METADATA_CACHE: Lazy<ModelMetadataCache> =
