...
pub async fn import_and_initialize_database(
    ...
    // An imported legacy database may already carry a transcription language
...
pub async fn initialize_fresh_database(app: AppHandle) -> Result<(), String> {
    ...
    // Default Summary Model: Built-in AI, sized to this machine (falls back to the
    // fast Gemma tier if hardware probing fails).
    let default_summary_model =
        crate::summary::summary_engine::commands::builtin_ai_get_recommended_model()
            .await
            .unwrap_or_else(|e| {
                error!(
                    "Failed to determine recommended summary model, defaulting to gemma3:1b: {}",
                    e
                );
                "gemma3:1b".to_string()
            });
