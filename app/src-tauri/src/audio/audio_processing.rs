...
pub fn resample(input: &[f32], from_sample_rate: u32, to_sample_rate: u32) -> Result<Vec<f32>> {
    ...
    waves_out
        .into_iter()
        .next()
        .ok_or_else(|| anyhow::anyhow!("Resampler produced no output channel"))
}

/// Infallible resampling wrapper.
///
/// WARNING: on failure this returns the ORIGINAL audio unchanged, which is wrong
/// when `from != to` (the caller gets audio at the wrong rate). Prefer the
/// fallible [`resample`] anywhere the result is fed to transcription; this
/// wrapper exists only for non-critical callers that cannot propagate an error.
/// The fallback is logged at `error!` so silent corruption is at least visible.
pub fn resample_audio(input: &[f32], from_sample_rate: u32, to_sample_rate: u32) -> Vec<f32> {
    match resample(input, from_sample_rate, to_sample_rate) {
        ...
        Err(e) => {
            error!(
                "Resampling {}Hz -> {}Hz failed: {}; returning input unchanged (output rate will be wrong)",
