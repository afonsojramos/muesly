...
pub struct ContinuousVadProcessor {
    ...
    // True when part of the current utterance was already force-emitted at the
    // max-duration cap; the eventual SpeechEnd must then emit only the tail.
    force_emitted_in_utterance: bool,
    // State tracking for smart logging
    last_logged_state: bool,
    // Observability: counters for a periodic activity log so silent VAD (many
    // windows processed but no speech detected for a long stretch) is visible
    // instead of failing quietly.
    obs_window_samples: usize,
    obs_chunks: usize,
    obs_speech_chunks: usize,
}
...
impl ContinuousVadProcessor {
    pub fn new(input_sample_rate: u32, redemption_time_ms: u32) -> Result<Self> {
        ...
        Ok(Self {
            ...
            force_emitted_in_utterance: false,
            // Initialize state tracking
            last_logged_state: false,
            obs_window_samples: 0,
            obs_chunks: 0,
            obs_speech_chunks: 0,
        })
    }
    ...
    pub fn flush(&mut self) -> Result<Vec<SpeechSegment>> {
        ...
        if self.in_speech && !self.current_speech.is_empty() {
            ...
            self.force_emitted_in_utterance = false;
        }
    ...
    fn process_chunk(&mut self, chunk: &[f32]) -> Result<()> {
        ...
        for transition in transitions {
            match transition {
                VadTransition::SpeechStart { timestamp_ms } => {
                    ...
                    self.in_speech = true;
                    // Silero's SpeechStart timestamp is already session-absolute;
                    // convert ms -> 16kHz samples directly. (Adding processed_samples
                    // on top double-counted and inflated force-emit/flush start times.)
                ...
                VadTransition::SpeechEnd { start_timestamp_ms, end_timestamp_ms, samples } => {
                    ...
                    // Use samples from VAD transition if available, otherwise use
                    // accumulated samples. After a max-duration force-emit, silero
                    // has not seen an intervening SpeechEnd, so the transition's
                    // samples span the utterance since its original start and would
                    // duplicate audio we already emitted (duplicated transcript
                    // text); emit only the accumulated tail in that case.
                    let speech_samples = if self.force_emitted_in_utterance {
                    ...
                    // For the tail, derive both timestamps from our own sample
                    // counts; silero's end includes post-speech padding measured on
                    // the full span and can disagree with the tail start.
                    let (segment_start_ms, segment_end_ms) = if self.force_emitted_in_utterance {
                        let start = (self.speech_start_sample as f64 / 16_000.0) * 1000.0;
                        (start, start + (speech_samples.len() as f64 / 16.0))
                    } else {
                    ...
                    if !speech_samples.is_empty() {
                        ...
                        info!("VAD: Completed speech segment: {:.1}ms duration, {} samples",
                              segment_end_ms - segment_start_ms, segment.samples.len());
        ...
        if self.in_speech {
            ...
            // CONTINUOUS-AUDIO FIX: with no silence gaps (e.g. a playing video or
            // back-to-back speech) the VAD never emits a `SpeechEnd`, so without a
            // cap `current_speech` would grow until `flush()` at stop — leaving the
            // meeting with a single giant segment and no live transcription.
            // Force-emit at a max duration and keep accumulating the same utterance.
            const MAX_SEGMENT_SAMPLES: usize = 16_000 * 15; // 15s at the 16kHz VAD rate
            if self.current_speech.len() >= MAX_SEGMENT_SAMPLES {
                ...
                let end_ms = start_ms + (samples.len() as f64 / 16_000.0) * 1000.0;

                info!(
                    "VAD: Force-emitting max-duration segment (continuous speech): {:.1}ms, {} samples",
                    end_ms - start_ms,
                    samples.len()
                );
                ...
                self.speech_segments.push_back(SpeechSegment {
                    ...
                    start_timestamp_ms: start_ms,
                    end_timestamp_ms: end_ms,
                    confidence: 0.8, // Estimated; not a natural speech boundary
                });

                // Continue the same speech run: next segment starts after this chunk.
                self.speech_start_sample = self.processed_samples + chunk.len();
                self.force_emitted_in_utterance = true;
            }
        }

        self.processed_samples += chunk.len();

        // Periodic VAD activity log (~every 60s of processed audio). Makes silent
        // stretches observable: if windows keep processing but speech stays at 0%,
        // the mic/system input is likely dead.
        self.obs_window_samples += chunk.len();
        self.obs_chunks += 1;
        if self.in_speech {
            self.obs_speech_chunks += 1;
        }
        const OBS_WINDOW_SAMPLES: usize = 16_000 * 60; // 60s at the 16kHz VAD rate
        if self.obs_window_samples >= OBS_WINDOW_SAMPLES {
            let active_pct = if self.obs_chunks > 0 {
                (self.obs_speech_chunks as f64 / self.obs_chunks as f64) * 100.0
            } else {
                0.0
            };
            info!(
                "VAD activity (last ~60s): {} windows processed, {} with speech ({:.0}% active), in_speech={}",
...
mod tests {
    ...
    fn test_vad_large_file_progress() {
        ...
        // The synthetic signal is sine-wave bursts, not real speech, so the Silero
        // ML model does not segment it into the 12 nominal bursts — asserting an
        // exact/high count is unreliable. What this test validates is that the
        // large-file chunked path runs end-to-end, detects the speech-like signal,
        // and reports progress. (The single-vs-chunked equivalence is covered by
        // test_chunked_matches_single.)
        assert!(!segments.is_empty(), "Expected at least one speech segment, found none");
...
mod repro_tests {
    ...
    /// Offline repro: replay a real failing recording through the exact
    /// pipeline path (600ms 48kHz windows -> ContinuousVadProcessor).
    /// Decode the meeting audio first:
    ///   ffmpeg -i audio.mp4 -f f32le -ac 1 -ar 48000 /tmp/vad_repro_48k.raw
    /// Run with: cargo test vad_repro_real_recording -- --nocapture --ignored
    #[test]
    #[ignore]
    fn vad_repro_real_recording() {
        let raw = std::fs::read("/tmp/vad_repro_48k.raw").expect("decode the recording first");
        let samples: Vec<f32> = raw
            .chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]]))
            .collect();
        println!(
            "Loaded {} samples ({:.1}s at 48kHz)",
            samples.len(),
            samples.len() as f64 / 48000.0
        );

        let mut processor = ContinuousVadProcessor::new(48000, 2000).expect("create VAD");
        let window = 28800; // 600ms at 48kHz, matches pipeline mixing window
        let mut total_segments = 0usize;

        for (i, win) in samples.chunks(window).enumerate() {
            let segments = processor.process_audio(win).expect("process");
            for s in &segments {
                total_segments += 1;
                println!(
                    "window {}: segment {:.0}ms..{:.0}ms ({} samples)",
                    i,
        ...
        let flushed = processor.flush().expect("flush");
        for s in &flushed {
            total_segments += 1;
            println!(
                "flush: segment {:.0}ms..{:.0}ms ({} samples)",
                s.start_timestamp_ms,
                s.end_timestamp_ms,
                s.samples.len()
            );
    ...
    fn load_raw(path: &str) -> Vec<f32> {
        std::fs::read(path)
            .expect("decode the recording first")
            .chunks_exact(4)
            .map(|b| f32::from_le_bytes([b[0], b[1], b[2], b[3]]))
            .collect()
    }
    ...
    fn prob_stats(label: &str, audio_16k: &[f32]) {
        ...
        let mut over_pos = 0usize;
        let mut frames = 0usize;
        for frame in audio_16k.chunks_exact(480) {
            let value = session.forward(frame.to_vec()).expect("forward");
            let prob = *value.try_extract_array::<f32>().unwrap().first().unwrap();
            frames += 1;
            if prob > max_prob {
                max_prob = prob;
            }
        ...
        println!(
            "{}: frames={} max_prob={:.3} frames>0.5={} rms={:.5} peak={:.3}",
            label, frames, max_prob, over_pos, rms, peak
        );
    }

    /// Compare speech probabilities on the custom-resampled audio vs an
    /// ffmpeg-resampled reference of the same recording.
    /// Run with: cargo test vad_probability_diag -- --nocapture --ignored
    #[test]
    #[ignore]
    fn vad_probability_diag() {
        let samples_48k = load_raw("/tmp/vad_repro_48k.raw");
        let processor = ContinuousVadProcessor::new(48000, 2000).expect("create VAD");
        let custom_16k = processor.resample_to_16k(&samples_48k).expect("resample");
        prob_stats("custom-resampler", &custom_16k);

        let ffmpeg_16k = load_raw("/tmp/vad_repro_16k.raw");
        prob_stats("ffmpeg-resampler", &ffmpeg_16k);
    }
}
