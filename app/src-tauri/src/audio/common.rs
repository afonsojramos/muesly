...
/// Ensure the global Whisper engine has `target_model` loaded, loading it if a
/// different (or no) model is currently loaded, and return the engine handle.
///
/// Shared by the import and retranscription flows; each resolves which model to
/// load with its own (lenient vs strict) policy, then calls this to perform the
/// identical lookup → discover → load sequence.
pub(crate) async fn ensure_whisper_model(target_model: &str) -> Result<Arc<WhisperEngine>> {
    ...
    if current_model.as_deref() != Some(target_model) {
        ...
        engine
            ...
            .await
            .map_err(|e| anyhow!("Failed to load Whisper model '{}': {}", target_model, e))?;
    }
    ...
    transcripts
        ...
        .map(|(text, start_ms, end_ms)| {
            ...
            TranscriptSegment {
                ...
                // Imported/retranscribed audio is a single mixed file; no source split.
                speaker: None,
            }
        })
        .collect()
...
/// Atomically write a JSON value to `folder/filename` via a temp file + rename.
///
/// Shared by the transcript/metadata writers so the temp-write-then-rename dance
/// (and the `.{filename}.tmp` naming) lives in one place. Note: the two
/// `TranscriptSegment` types are intentionally NOT merged — `api::TranscriptSegment`
/// is the import/export shape, while `recording_saver::TranscriptSegment` is an
/// IPC contract returned by `get_transcript_history`; only the write mechanics are shared.
pub(crate) fn write_json_atomic(
    ...
    if !temp_path.exists() {
        anyhow::bail!("Temp file verification failed: {}", temp_path.display());
    }
...
pub(crate) fn write_transcripts_json(folder: &Path, segments: &[TranscriptSegment]) -> Result<()> {
    ...
    write_json_atomic(folder, "transcripts.json", &json)?;

    info!(
        "Wrote transcripts.json with {} segments to {}",
        segments.len(),
        folder.join("transcripts.json").display()
    );
    Ok(())
}
