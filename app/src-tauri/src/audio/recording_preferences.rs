...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\muesly-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("muesly-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("muesly-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/muesly-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("muesly-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/muesly-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("muesly-recordings")
    }
