...
fn find_ffmpeg_path_internal() -> Option<PathBuf> {
    ...
    let installation_dir = match sidecar_dir() {
        Ok(dir) => dir,
        Err(e) => {
            error!("failed to resolve sidecar directory: {}", e);
            return None;
        }
    };
...
fn get_ffmpeg_install_dir() -> Result<PathBuf, anyhow::Error> {
    ...
    // Create directory if it doesn't exist. We deliberately do NOT modify the
    // user's shell rc files: the app always invokes ffmpeg via the absolute path
    // returned from here, so editing PATH was both unnecessary and a surprising,
    // non-atomic mutation of the user's environment.
    if !local_bin.exists() {
