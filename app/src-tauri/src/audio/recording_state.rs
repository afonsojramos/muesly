...
///
/// Recording state is touched from the audio callback threads; a panic while
/// holding one of these locks should not poison it and cascade-panic every
/// later access. Recovering the inner value (the data is still consistent for
/// our purposes) degrades gracefully instead.
trait LockExt<T> {
...
pub struct RecordingState {
    ...
    // Loudest microphone sample (abs amplitude, 0.0..1.0) seen since the current
...
impl RecordingState {
    ...
    pub fn start_recording(&self) -> Result<()> {
        ...
        *self.recording_start.lock_recover() = Some(Instant::now());
    ...
    /// Record a microphone chunk's peak amplitude (abs, 0.0..1.0), keeping the
    ...
    pub fn note_mic_amplitude(&self, peak: f32) {
        ...
        let mut prev = self.peak_mic_amplitude.load(Ordering::Relaxed);
        while peak > f32::from_bits(prev) {
            match self.peak_mic_amplitude.compare_exchange_weak(
                prev,
                new_bits,
                Ordering::Relaxed,
                Ordering::Relaxed,
    ...
    pub fn stop_recording(&self) {
        ...
        *self.pause_start.lock_recover() = None;
        ...
        *self.audio_sender.lock_recover() = None;
        ...
        *self.microphone_device.lock_recover() = None;
        *self.system_device.lock_recover() = None;
        *self.disconnected_device.lock_recover() = None;
    ...
    pub fn pause_recording(&self) -> Result<()> {
        ...
        *self.pause_start.lock_recover() = Some(Instant::now());
    ...
    pub fn start_reconnecting(&self, device: Arc<AudioDevice>, device_type: DeviceType) {
        ...
        *self.disconnected_device.lock_recover() = Some((device, device_type));
    ...
    {
        *self.error_callback.lock_recover() = Some(Box::new(callback));
    }
    ...
    pub fn report_error(&self, error: AudioError) {
        ...
        *self.last_error.lock_recover() = Some(error.clone());

        // Call error callback if set
        if let Some(callback) = self.error_callback.lock_recover().as_ref() {
            callback(&error);
        }
    ...
    pub fn get_active_recording_duration(&self) -> Option<f64> {
        self.recording_start.lock_recover().map(|start| {
    ...
    pub fn get_total_pause_duration(&self) -> f64 {
        self.total_pause_duration.lock_recover().as_secs_f64()
    }
    ...
    pub fn cleanup(&self) {
        ...
        *self.audio_sender.lock_recover() = None;
        *self.last_error.lock_recover() = None;
        *self.error_callback.lock_recover() = None;
        *self.stats.lock_recover() = RecordingStats::default();
        *self.recording_start.lock_recover() = None;
        *self.pause_start.lock_recover() = None;
        *self.total_pause_duration.lock_recover() = std::time::Duration::ZERO;
        self.error_count.store(0, Ordering::SeqCst);
        self.recoverable_error_count.store(0, Ordering::SeqCst);
...
mod tests {
    ...
    fn duration_getters_survive_a_poisoned_lock() {
        ...
        let _ = std::thread::spawn(move || {
            let _guard = s.recording_start.lock().unwrap();
            panic!("intentional poison");
        })
        .join();

        // These must not panic now that the lock is poisoned.
        let _ = state.get_recording_duration();
        let _ = state.get_active_recording_duration();
