...
pub fn calculate_buffer_timeout(
    ...
    // Calculate base timeout (ms) from reported buffer size, add 2x headroom for
    // jitter (Cap's strategy), and round to whole milliseconds. Computing in
    // integer ms avoids float artifacts like 159.999996ms for an intended 160ms.
    let base_ms = (buffer_size as f64 / sample_rate as f64) * 1000.0;
    let with_headroom = Duration::from_millis((base_ms * 2.0).round() as u64);
...
mod tests {
    ...
    #[test]
    fn test_name_heuristic_fallthrough_to_unknown() {
        // A device name with no Bluetooth pattern and no buffer-size info should
        // fall through every detection layer to Unknown. Use a synthetic name that
        // is not a real Core Audio device — otherwise on macOS the native
        // transport-type layer would (correctly) classify a real built-in mic as
        // Wired, making this test machine-dependent.
        let kind = InputDeviceKind::detect("Synthetic Test Device 9000", 0, 0);
