...
// `parakeet_engine::ModelStatus` path.
...
pub struct ParakeetEngine {
    models_dir: PathBuf,
    // The model is wrapped in `Arc<std::sync::Mutex<_>>` so a handle can be cloned
    // out from under the tokio `RwLock` and `transcribe_samples` (which needs
    // `&mut` and blocks on ONNX inference) can run inside `spawn_blocking`. This
...
impl ParakeetEngine {
    ...
    pub async fn transcribe_audio(&self, audio_data: Vec<f32>) -> Result<String> {
        *self.last_used.write().await = std::time::Instant::now();
        // Clone the model handle and release the tokio read lock before inference,
        // so load/unload aren't blocked for the duration of the ONNX call.
        let model = {
        ...
        // ONNX inference blocks; run it off the async runtime. Concurrent calls
        // serialize on the inner mutex (the model requires `&mut`), but the tokio
        // runtime and model load/unload remain responsive.
