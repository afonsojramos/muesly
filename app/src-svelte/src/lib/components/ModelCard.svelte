		...
		<div class="mb-2 flex items-start justify-between">
			...
			<div class="ml-4 flex items-center gap-2">
				{#if isAvailable}
					...
					{#if isHovered}
						<Tooltip label="Delete model to free up space">
							{#snippet trigger()}
								<button
									in:fly={{ duration: 150 }}
									onclick={(e) => {
				...
				{:else if isCorrupted}
					<div class="flex gap-2">
						<button
							...
							class="rounded-md bg-orange-600 px-3 py-1.5 text-sm font-medium text-white transition-colors hover:bg-orange-700"
						>
							Delete
						</button>
						<button
