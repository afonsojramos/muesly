...
interface SummaryPollResult {
	status: string;
	error?: string;
	data?: Record<string, unknown> & { markdown?: string; MeetingName?: string; _section_order?: string[] };
	meetingName?: string;
	[key: string]: unknown;
}
	...
	const processSummary = async ({
		...
		try {
			...
			const timeSinceRecording =
				(Date.now() - new Date(meeting.created_at).getTime()) / 60000;
			...
			sidebar.startSummaryPolling(meeting.id, processId, async (pollingResult: SummaryPollResult) => {
