...
- Icon-only controls: every button or interactive element whose only visible content is an icon must have a visible tooltip naming the action, plus an `aria-label`. Use the shared `IconButton` component (`app/src-svelte/src/lib/components/IconButton.svelte`), which enforces both via a single `label` prop; for non-button triggers, wrap with the shared `Tooltip` primitives instead. Never ship bare `size="icon*"` `Button`s; tooltips are not optional decoration, they are how the control is discovered and understood.
...
- Background tasks (`background-tasks.svelte.ts`): entries track work that outlives any view, so they must be driven by global Tauri events (or store-owned polling), never by view lifecycles — a navigation/unmount may detach a view's update callback but must never stop tracking. The store enforces this: `dismiss()` refuses running tasks; flows that genuinely stop work use `cancel()`.
