...
* **Calendar** (`calendar/`): macOS-only local calendar integration via EventKit (`objc2-event-kit`). At record time the meeting happening "now" is matched (a pure, platform-free scorer in `matching.rs`) and, for high-confidence matches, used to title the recording; a redacted snapshot is stored in the `calendar_events` table and injected into the summary prompt as a `<meeting_context>` block. Reading is entirely on-device (no account, no network), covering whatever accounts the user has added to macOS Calendar. Attendee emails are never stored; for cloud summary providers, attendee names and notes are withheld by default (opt-in toggles) and the conference URL is stripped, gated by an `LLMProvider::data_egress` classifier. EventKit authorization is requested on the main thread with the completion handled off-main; fetches run off-main.
