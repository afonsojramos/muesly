...
* **Semantic Search** (`embedding_engine.rs`, `embedding_indexer.rs`): fully on-device hybrid retrieval behind the "Ask your meetings" chat search. A small multilingual embedding model (`multilingual-e5-small` int8 ONNX, 384 dims, downloaded once from a pinned revision with SHA-256 verification) runs through the same in-process ONNX Runtime as Parakeet. Transcript and summary chunks are embedded (mean-pooled, L2-normalized) into the `meeting_embeddings` table and kept current by fire-and-forget triggers (recording save, retranscription, summary completion) plus a startup backfill sweep. Meetings with no indexable text get a zero-chunk marker row so the backfill sweep retires them instead of re-inspecting them on every startup. At query time the chat's `tool_search` fuses the unchanged FTS5/title ranking with a brute-force cosine scan via reciprocal rank fusion; semantic transcript hits surface their chunk's recording moment as a `[MM:SS]` snippet prefix so answers can cite it. When the model is absent, behavior degrades to keyword-only search exactly. The embedder unloads after idle like the transcription models.
* **Database** (`database/`): Local SQLite via `sqlx` with `runtime-tokio`. Repositories cover meetings, transcripts, notes (`meeting_notes`, saved via `api_save_meeting_notes` / `api_get_meeting_notes`), calendar snapshots (`calendar_events`), embedding chunks (`meeting_embeddings`), and settings. Migrations run at app startup.
